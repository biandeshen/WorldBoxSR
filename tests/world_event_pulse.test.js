...
test('world event pulse prioritizes political events and aggregates life events', () => {
  const events = [
    ...
    { id: 15, day: 14450, type: 'polity.founded', polityId: 4, name: 'Stonevale Realm' }
  ];
  ...
  assert.equal(cards[0].kind, 'polity.founded');
  assert.equal(cards[0].title, 'Stonevale Realm rose as a power');
  assert.equal(cards[1].kind, 'settlement.founded');
  assert.equal(cards[2].kind, 'human.deaths');
  assert.equal(cards[3].kind, 'human.births');
  assert.equal(cards[3].title, '2 births');
});
