...
const SIZE = 16;
const YEARS = 300;
const SEEDS = Array.from({ length: 60 }, (_, i) => 61 + i);
const COUNTS = [2, 4];
...
test('temporary fresh encounter-safe compact placement validation', () => {
  const rows = [];
  ...
  for (const founders of COUNTS) {
    for (const seed of SEEDS) {
      ...
      if (baseline.extinctionYear !== null && intervention.extinctionYear === null) rescuedCases.push(pairCase(baseline, intervention));
      if (baseline.extinctionYear === null && intervention.extinctionYear !== null) harmedCases.push(pairCase(baseline, intervention));
    }
  }

  const count2 = summaries.find((s) => s.founders === 2);
  const count4 = summaries.find((s) => s.founders === 4);
  const totalRescued = summaries.reduce((sum, s) => sum + s.rescued, 0);
  const totalHarmed = summaries.reduce((sum, s) => sum + s.harmed, 0);
  const validationPass = (
    count2.interventionExtinctions <= count2.baselineExtinctions
    && count2.interventionZeroBirthExtinctions <= count2.baselineZeroBirthExtinctions
    && count4.interventionExtinctions <= count4.baselineExtinctions
    && totalRescued > totalHarmed
  );
  ...
  console.log(`GRAZER_ENCOUNTER_SAFE_PLACEMENT_FRESH ${JSON.stringify(result)}`);
});
...
function runArm(seed, founders, arm) {
  ...
  const initialMeanLocalVegetationUtilization = round(mean(selected.map((cell) => localVegetationUtilization(world, cell.x, cell.y))));
  ...
  for (let elapsed = 0; elapsed < totalDays; elapsed += 1) {
    ...
    if (newBirths > 0 || newDeaths > 0) {
      for (const event of newEventsSince(world, eventIdBefore)) {
        if (event.type === 'creature.born' && event.parentCreatureIds?.some((id) => id > founders)) replacementParentBirths += 1;
    ...
    if (world.day % SAMPLE_INTERVAL_DAYS === 0) minVegetationUtilization = Math.min(minVegetationUtilization, summarizeWorld(world).vegetationUtilization);
  ...
  assert.deepEqual(world.rng.snapshot(), rngBefore, `${seed}/${founders}/${arm} consumed sequential RNG`);
...
function summarizePairs(rows, founders) {
  let rescued = 0, harmed = 0, bothExtinct = 0, bothAlive = 0;
  let baselineZeroBirthExtinctions = 0, interventionZeroBirthExtinctions = 0;
  let baselineInitialZeroPair = 0, interventionInitialZeroPair = 0;
  const firstBirthDeltas = [], baselineMinVeg = [], interventionMinVeg = [], baselineStarvation = [], interventionStarvation = [];

  for (const seed of SEEDS) {
    const baseline = rowFor(rows, seed, founders, 'baseline');
    const intervention = rowFor(rows, seed, founders, 'encounter-safe');
    const bExtinct = baseline.extinctionYear !== null;
    const iExtinct = intervention.extinctionYear !== null;
    if (bExtinct && !iExtinct) rescued += 1;
    else if (!bExtinct && iExtinct) harmed += 1;
    else if (bExtinct && iExtinct) bothExtinct += 1;
    else bothAlive += 1;
    if (bExtinct && baseline.totalBirths === 0) baselineZeroBirthExtinctions += 1;
    if (iExtinct && intervention.totalBirths === 0) interventionZeroBirthExtinctions += 1;
    if (baseline.initialPairEdges === 0) baselineInitialZeroPair += 1;
    if (intervention.initialPairEdges === 0) interventionInitialZeroPair += 1;
    if (baseline.firstBirthYear !== null && intervention.firstBirthYear !== null) firstBirthDeltas.push(intervention.firstBirthYear - baseline.firstBirthYear);
    baselineMinVeg.push(baseline.minVegetationUtilization);
    interventionMinVeg.push(intervention.minVegetationUtilization);
  ...
  return {
    ...
    medianMinVegetationBaseline: round(median(baselineMinVeg)),
    medianMinVegetationIntervention: round(median(interventionMinVeg)),
...
function seedFounders(world, count, arm) {
  ...
  const selected = arm === 'baseline' ? candidates.slice(0, count) : selectEncounterSafe(candidates, count);
...
// Frozen Sprint029 algorithm. Do not change during validation.
function selectEncounterSafe(candidates, count) {
  ...
  while (selected.length < count) {
    let next = candidates.find((candidate) => !selectedSet.has(candidate) && selected.some((cell) => chebyshev(cell, candidate) <= PARTNER_RADIUS));
...
function founderGraph(cells) {
  ...
  for (let i = 0; i < cells.length; i += 1) {
    for (let j = i + 1; j < cells.length; j += 1) {
      if (chebyshev(cells[i], cells[j]) <= PARTNER_RADIUS) {
        adjacency[i].push(j); adjacency[j].push(i); edges += 1;
      }
    }
  }
  ...
  for (let start = 0; start < cells.length; start += 1) {
    ...
    while (stack.length) {
      ...
      for (const neighbor of adjacency[current]) if (!visited.has(neighbor)) { visited.add(neighbor); stack.push(neighbor); }
    }
  }
  return { edges, components, isolates: adjacency.filter((neighbors) => neighbors.length === 0).length };
}

function pairCase(baseline, intervention) { return { baseline: compactRow(baseline), intervention: compactRow(intervention) }; }
function compactRow(row) { return { seed: row.seed, founders: row.founders, arm: row.arm, initialPairEdges: row.initialPairEdges, initialGraphComponents: row.initialGraphComponents, initialIsolates: row.initialIsolates, initialMeanLocalVegetationUtilization: row.initialMeanLocalVegetationUtilization, firstBirthYear: row.firstBirthYear, birthsByYear20: row.birthsByYear20, totalBirths: row.totalBirths, extinctionYear: row.extinctionYear, finalPopulation: row.finalPopulation, starvationDeaths: row.starvationDeaths, oldAgeDeaths: row.oldAgeDeaths, minVegetationUtilization: row.minVegetationUtilization, maxLiving: row.maxLiving }; }
function rowFor(rows, seed, founders, arm) { return rows.find((row) => row.seed === seed && row.founders === founders && row.arm === arm); }
function newEventsSince(world, eventIdBefore) { const events = []; for (let i = world.history.length - 1; i >= 0; i -= 1) { const event = world.history[i]; if (event.id < eventIdBefore) break; events.push(event); } return events; }
function localVegetationUtilization(world, x, y) { const cells = [tileAt(world, x, y), ...passableNeighbors8(world, x, y)].filter((tile) => tile.passable); const vegetation = cells.reduce((s, t) => s + t.vegetation, 0); const capacity = cells.reduce((s, t) => s + t.vegetationCapacity, 0); return capacity > 0 ? vegetation / capacity : 0; }
function chebyshev(a, b) { return Math.max(Math.abs(a.x - b.x), Math.abs(a.y - b.y)); }
function median(values) { assert.ok(values.length > 0); const sorted = [...values].sort((a, b) => a - b); const m = Math.floor(sorted.length / 2); return sorted.length % 2 ? sorted[m] : (sorted[m - 1] + sorted[m]) / 2; }
