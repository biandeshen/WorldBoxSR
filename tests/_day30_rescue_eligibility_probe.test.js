...
import { evaluateDay30Rules } from '../engine/analysis/reattachment_classifier.js';
...
test('temporary 100-year day30 rescue eligibility probe', () => {
  for (const seed of seeds) {
    ...
    const evaluation = evaluateDay30Rules(rows);
    console.log(`DAY30_RESCUE_RULES ${JSON.stringify({
      seed,
      ...
      rules: evaluation.rules.map((rule) => ({
        ...
        fpFast: rule.fastRecoveryFalsePositives,
        ...
        recoveryInterference: round(rule.naturalRecoveryInterferenceRate),
