...
class WorldScene extends Phaser.Scene {
  ...
  create() {
    try {
      ...
      this.warbands = new WarbandLayer(this, TILE_SIZE);
      this.bindInput();
      this.bindDom();
  ...
  update(time) {
    if (!this.world || !this.entities || !this.warbands) return;
  ...
  resetWorld() {
    ...
    this.storyCursorId = latestHistoryEventId(this.world);
  ...
  syncWorld(now) {
    ...
    this.warbands.sync(this.view, now, Math.min(220, STEP_INTERVAL_MS * 1.7));
    this.refreshChronicle();
    this.refreshCivilizationPulse();
  }
  ...
  useTool(x, y, count) {
    try {
      ...
      this.showToast(toolMessage(result, count));
    } catch (error) {
  ...
  inspectTile(x, y) {
    ...
    if (selection.kind === 'warband') {
      const polity = this.world.polities.find((candidate) => candidate.id === target.polityId);
      const opponent = this.world.polities.find((candidate) => candidate.id === target.opponentPolityId);
      const targetSettlement = this.world.settlements.find((candidate) => candidate.id === target.targetSettlementId);
      const opposingBand = (this.world.warbands ?? []).find((candidate) => candidate.active && candidate.relationKey === target.relationKey && candidate.warStartedDay === target.warStartedDay && candidate.polityId === target.opponentPolityId);
      const engaged = Boolean(opposingBand && Math.abs(target.x - opposingBand.x) + Math.abs(target.y - opposingBand.y) <= 1);
      inspector.textContent = [
        `⚑ Warband #${target.id}`,
        `${polity?.name ?? `Polity #${target.polityId}`} vs ${opponent?.name ?? `Polity #${target.opponentPolityId}`}`,
        `${engaged ? '⚔ engaged' : 'marching'} · strength ${target.strength}/${target.initialStrength}`,
        `engagements ${target.engagements} · captures ${target.captures ?? 0} · mobilized from ${target.sourceAdultPopulation} adults`,
        targetSettlement ? `objective: ${targetSettlement.name}` : `objective settlement #${target.targetSettlementId}`,
        `tile ${target.x},${target.y}`
      ].join('\n');
      return;
    }
    if (selection.kind === 'human') {
      const settlement = Number.isInteger(target.settlementId) ? this.world.settlements.find((candidate) => candidate.id === target.settlementId) : null;
      const polity = Number.isInteger(settlement?.polityId) ? this.world.polities.find((candidate) => candidate.id === settlement.polityId) : null;
      inspector.textContent = [
        `Human #${target.id}`,
        `${target.sex === 'F' ? 'female' : 'male'} · age ${(target.ageDays / this.world.config.daysPerYear).toFixed(1)}y`,
        polity?.rulerId === target.id ? `♛ ruler of ${polity.name}` : (polity ? `citizen of ${polity.name}` : 'no polity'),
    ...
    if (selection.kind === 'creature') {
      inspector.textContent = [
        `${creatureSpeciesLabel(target.species)} #${target.id}`,
    ...
    if (selection.kind === 'settlement') {
      const polity = Number.isInteger(target.polityId) ? this.world.polities.find((candidate) => candidate.id === target.polityId) : null;
      const previousPolity = Number.isInteger(target.previousPolityId) ? this.world.polities.find((candidate) => candidate.id === target.previousPolityId) : null;
      const rebelledFrom = Number.isInteger(target.lastRebelledFromPolityId) ? this.world.polities.find((candidate) => candidate.id === target.lastRebelledFromPolityId) : null;
      const relation = polity ? relevantRelation(this.world, polity.id) : null;
      const counterpartId = relation ? (relation.polityAId === polity.id ? relation.polityBId : relation.polityAId) : null;
      const counterpart = Number.isInteger(counterpartId) ? this.world.polities.find((candidate) => candidate.id === counterpartId) : null;
      ...
      inspector.textContent = [
        ...
        rebellionEligibleYear && (target.rebellionCount ?? 0) < 1 ? `occupation · rebellion seed eligible Y${rebellionEligibleYear}` : '',
        rebellionYear ? `✦ rebelled Y${rebellionYear} from ${rebelledFrom?.name ?? `Polity #${target.lastRebelledFromPolityId}`}` : '',
        relation ? `${relation.atWar ? '⚔ WAR' : relation.stance.toUpperCase()} with ${counterpart?.name ?? `Polity #${counterpartId}`} · score ${relation.score}` : (polity ? 'diplomacy: no active counterpart' : ''),
        relation?.cause ? `pressure: ${relation.cause}` : '',
  ...
  refreshHud() {
    ...
    const activeWars = (this.world.relations ?? []).filter((relation) => relation.active && relation.atWar).length;
    const activeWarbands = (this.world.warbands ?? []).filter((warband) => warband.active).length;
    const livingWolves = this.world.creatures.filter((creature) => creature.alive && creature.species === 'wolf').length;
    stats.innerHTML = [
      `<span><b>Year ${summary.year.toFixed(1)}</b></span>`,
      `<span>👤 ${summary.population}</span>`,
      `<span>🐾 ${summary.grazers}</span>`,
      livingWolves > 0 ? `<span>🐺 ${livingWolves}</span>` : '',
      `<span>♛ ${activePolities}</span>`,
      `<span>⚔ ${activeWars}</span>`,
      `<span>⚑ ${activeWarbands}</span>`,
  ...
  refreshChronicle() {
    ...
    historyScopeLabel.textContent = 'World chronicle · authoritative history';
    const rows = civilizationChronicle(this.world, { limit: 7 });
    ...
    historyList.querySelectorAll('button[data-event-id]').forEach((button) => {
      button.addEventListener('click', () => {
        const entry = rows.find((candidate) => String(candidate.eventId) === button.dataset.eventId);
        if (historyDetail) historyDetail.textContent = entry ? formatChronicleDetail(entry) : 'Event unavailable';
      });
    });
  }

  refreshCivilizationPulse() {
    if (!this.world) return;
    const latestId = latestHistoryEventId(this.world);
    if (this.booting) {
      this.storyCursorId = latestId;
      return;
    }
    const pulse = latestCivilizationPulse(this.world, { afterEventId: this.storyCursorId });
    this.storyCursorId = latestId;
    if (pulse) this.showToast(`${pulse.icon} ${pulse.headline}`);
  }
...
function relevantRelation(world, polityId) {
  return (world.relations ?? [])
    .filter((relation) => relation.active && (relation.polityAId === polityId || relation.polityBId === polityId))
...
function creatureSpeciesLabel(species) {
  ...
  const text = String(species ?? 'Creature').replaceAll('_', ' ');
  return text.length ? `${text[0].toUpperCase()}${text.slice(1)}` : 'Creature';
}

function toolMessage(result, count) {
  if (result.effect === 'rain') {
    if (result.noEffect) return '☂ Rain fell · area already saturated';
    return `☂ Rain restored · +${result.vegetationAdded.toFixed(1)} vegetation · +${result.foodAdded.toFixed(1)} food`;
  }
  if (result.effect === 'meteor') {
    if (result.noEffect) return '☄ Meteor struck · no living targets or vegetation';
    const lives = (result.humanIds?.length ?? 0) + (result.creatureIds?.length ?? 0);
    return `☄ Meteor impact · ${lives} ${lives === 1 ? 'life' : 'lives'} · ${result.vegetationRemoved.toFixed(1)} vegetation`;
  }
  if (result.effect === 'lightning') return '⚡ Lightning struck';
  if (result.effect === 'erase') return '✕ Erase applied';
  if (result.effect === 'spawn_grazer') return `🐾 Spawned ${count} grazer${count === 1 ? '' : 's'}`;
  if (result.effect === 'spawn_wolf') return `Spawned ${count} wolf${count === 1 ? '' : 's'}`;
