...
function settlementSignature(settlement) {
  return [
    ...
    settlement.polityId ?? 'none',
    settlement.polityName ?? 'none',
    settlement.polityColorIndex ?? 'none',
    settlement.polityBannerStyle ?? 'none',
    settlement.isCapital ? 'capital' : 'member'
  ].join('|');
}
...
function createSettlementVisual(scene, settlement, tileSize) {
  ...
  ground.setStrokeStyle(1.2, settlement.active ? color : 0x80796e, settlement.active ? 0.48 : 0.2);
  ...
  const primaryName = settlement.isCapital && settlement.polityName
    ? `♛ ${settlement.polityName}`
    : settlement.name;
  const secondaryText = settlement.polityName
    ? (settlement.isCapital ? `${settlement.name} · ⌂ ${settlement.population}` : `${settlement.polityName} · ⌂ ${settlement.population}`)
    : `⌂ ${settlement.population}`;

  const label = scene.add.text(0, -28 - tier, primaryName, {
  ...
  const population = scene.add.text(0, -25 - tier, secondaryText, {
...
function createBanner(scene, settlement, tier, color) {
  const x = 15 + tier * 1.4;
  const pole = scene.add.rectangle(x, -10, 1.2, 16, 0x59432f, 1);
  const flag = scene.add.rectangle(x + 4, -15.5, 8, 5, color, 1);
  const accent = lighten(color);
  const parts = [pole, flag];
  const style = settlement.polityBannerStyle || 'plain';

  if (style === 'stripe') {
    parts.push(scene.add.rectangle(x + 4, -15.5, 8, 1.2, accent, 0.92));
  } else if (style === 'split') {
    parts.push(scene.add.rectangle(x + 6, -15.5, 4, 5, accent, 0.6));
  } else if (style === 'cross') {
    parts.push(scene.add.rectangle(x + 4, -15.5, 1.2, 5, accent, 0.9));
    parts.push(scene.add.rectangle(x + 4, -15.5, 8, 1.1, accent, 0.9));
  } else if (style === 'chevron') {
    parts.push(scene.add.triangle(x + 2.6, -15.5, -2, -2.1, -2, 2.1, 1.7, 0, accent, 0.92));
  } else {
    parts.push(scene.add.rectangle(x + 2.4, -16.5, 3, 1.2, accent, 0.72));
  }
  return parts;
}
...
function createHall(scene, dx, dy, active, polityColorValue) {
  ...
  const roof = scene.add.triangle(dx, dy - 5.5, -8, 4.5, 8, 4.5, 0, -5, active ? mixColor(0x713b35, polityColorValue, 0.28) : 0x4c4b48, 1);
  const door = scene.add.rectangle(dx, dy + 3.8, 3.2, 5.4, 0x513628, 1);
  const crest = scene.add.rectangle(dx, dy - 2.8, 3, 3, active ? polityColorValue : 0x6d6861, 1);
